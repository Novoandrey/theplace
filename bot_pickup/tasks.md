...
| FR-19 (later) запоминание опций по умолчанию | задачи позже |
| FR-20 (later) повторить заказ + избранные | расширяет US-E1/T044; задачи позже |
| FR-21 (later) часы работы + заказ к времени + билет за N мин | конкретизирует US-A4/T027; задачи позже |

**Статус Analyze:** FR-1…FR-16 закрыты задачами (v0-core ✓); FR-17/19/20/21 — специфицированы, задачи
заведём при планировании соответствующего инкремента (вне v0-core); каждое архитектурное решение plan
(aiogram+FSM/Redis, Postgres+SQLAlchemy+Alembic, адаптеры `MenuSource`/`OrderSink`, нумерация, статусы,
заглушка оплаты, Docker/VPS, long-polling↔webhook) отражено в задачах; задачи не выходят за структуру
plan §11.

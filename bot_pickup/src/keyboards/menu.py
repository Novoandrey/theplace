...
from src.cart.cart import format_kopecks, unit_kopecks
...
class AddCB(CallbackData, prefix="add"):
    id: str  # добавить позицию без опций
...
def kb_item_card(menu: Menu, item: MenuItem, selected: dict[str, str]) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    if item.options:
        for gid in item.options:
            ...
            for ch in group.choices:
                mark = ru.MARK_SELECTED if selected.get(gid) == ch.id else ""
                row.append(
...
def item_card_text(menu: Menu, item: MenuItem, selected: dict[str, str]) -> str:
    parts = [item.title]
    if item.serving:
        parts.append(item.serving)
    parts.append(format_kopecks(unit_kopecks(menu, item.id, selected)))
    text = " · ".join(parts)
    if item.description:
        text += f"\n{item.description}"
    return text
